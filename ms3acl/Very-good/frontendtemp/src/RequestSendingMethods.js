...
const addDeliveryAddress = async (touristId, addressData) => {
  try {
...
/*const selectDeliveryAddress = async (touristId, selectedAddress) => {
  const response = await fetch(`/api/tourists/${touristId}/select-delivery-address`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
  ...
  if (response.status === 200) {
    ...
    throw new Error(data.error || 'Failed to select address');
  }
};*/
...
export { registerTourist,fetchAllItineraries, fetchTouristByEmail, updateTouristByEmail, createTourGuideRequest, fetchTourGuideByEmail,fetchAllTags,updateTag,deleteTag,addAdmin,addTourismGoverner,
  ...
  rateactivity,bookItem ,deleteSeller,updateTourGuideByEmail ,updateTouristByEmailT,deletAdvertiser,deleteTourGuide ,getActivitieswithAdvertiserId,getTourGuideByEmail,fetchTourists, addDeliveryAddress,};
