...
void drawGround(float centerX, float centerY, float centerZ, float width, float depth) {
    glColor3f(0.0f, 0.9f, 0.0f);  // Greenish color for the ground
...
void drawPlayer() {
    ...
    glPushMatrix();
    glTranslatef(0.0f, 1.85f,0.0f);  // Position head above body
    ...
    // Draw right leg (cone)
    glPushMatrix();
    glTranslatef(0.3f, -0.5f, 10.0f);  // Position right leg
    glRotatef(90.0f, 1.0f, 0.0f, 0.0f);  // Rotate leg to position
